 ...
 * The model's value (if any) is REPLACED. Inputs are derived from the
 * parsed payload alone (n from `metricSource.n`, recencyDays=0,
 * deviationStdRatio=null) — the route layer doesn't pass bucket-series
 * data through here today. v1.4.17's feedback ratchet will need a
 * resolver injection; re-introduce one when the second caller exists.
 *
 * Emits a Wide-Event with per-rec model-vs-computed pairs so admin
 * observability can chart drift between what the LLM thinks and what
...
export async function generateInsight(
  ...
  if (first.ok && first.parsed) {
    ...
    applyConfidenceOverride(first.parsed);
    return {
      parsed: first.parsed,
      raw: first.raw,
      attempts: 1,
  ...
  if (second.ok && second.parsed) {
    ...
    applyConfidenceOverride(second.parsed);
    return {
