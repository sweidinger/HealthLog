/**
 * v1.11.0 — shared constants + mint helper for the WHOOP OAuth state nonce
 * ledger. Mirrors `src/lib/withings/oauth-state.ts`.
 *
 * The nonce is random + opaque; a short-lived `WhoopOAuthState` row carries
 ...
 * sameSite:lax at the connect route (mirrors the Withings cookie shape).
 */
export const WHOOP_OAUTH_STATE_COOKIE = "whoop_state" as const;

/**
 * 10-minute TTL on the state row. Matches the cookie `maxAge`. Long enough to
 * cover a user approving the WHOOP consent prompt; short enough that an
 * abandoned handshake doesn't strand a row for hours.
 */
export const WHOOP_OAUTH_STATE_TTL_MS = 10 * 60 * 1000;
 ...
 */
export function mintWhoopOAuthStateNonce(): string {
