...
describe("PUT /api/dashboard/widgets — heroVisible thread-through", () => {
  const knownId = DASHBOARD_WIDGET_IDS[0];

  it("persists an explicit heroVisible: false WITHOUT dropping stored chartOverlayPrefs", async () => {
    // The regression class this route already guards for
    // `chartOverlayPrefs`: a layout PUT that omits a sibling field must
    // not wipe it. heroVisible sent + prefs omitted → prefs merge from
    // the stored blob, heroVisible persists from the body.
    const storedPrefs = {
      bp: {
    ...
    vi.mocked(prisma.user.findUnique).mockResolvedValue({
      dashboardWidgetsJson: serializeDashboardLayout({
        version: 1,
        widgets: [{ id: knownId, visible: true, order: 0 }],
        chartOverlayPrefs: storedPrefs,
      }),
    } as never);
    vi.mocked(prisma.user.update).mockResolvedValue({} as never);
    ...
    expect(updateArg.data.dashboardWidgetsJson.chartOverlayPrefs).toEqual(
      storedPrefs,
    );

    // The response echoes the persisted value.
    const body = (await res.json()) as { data: { heroVisible: boolean } };
    expect(body.data.heroVisible).toBe(false);
  });

  it("preserves a stored heroVisible: false when the PUT omits the field", async () => {
    vi.mocked(prisma.user.findUnique).mockResolvedValue({
      dashboardWidgetsJson: serializeDashboardLayout({
        version: 1,
        widgets: [{ id: knownId, visible: true, order: 0 }],
  ...
  it("GET resolves heroVisible alongside the widget layout", async () => {
    vi.mocked(prisma.user.findUnique).mockResolvedValue({
      dashboardWidgetsJson: serializeDashboardLayout({
        version: 1,
        widgets: [{ id: knownId, visible: true, order: 0 }],
