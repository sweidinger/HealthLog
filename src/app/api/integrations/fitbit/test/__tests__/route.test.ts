...
beforeEach(() => {
  ...
  vi.mocked(prisma.fitbitConnection.findUnique).mockReset();
...
function emptyRequest(): Request {
  return new Request("http://localhost/api/integrations/fitbit/test", {
    method: "POST",
    headers: { "content-length": "0" },
  });
}

describe("POST /api/integrations/fitbit/test", () => {
  it("happy path returns ok with shape", async () => {
    ...
    vi.mocked(getValidToken).mockResolvedValueOnce({
      ...
      connection: { id: "fc-1", fitbitUserId: "fu-1" },
    });
    vi.mocked(prisma.fitbitConnection.findUnique).mockResolvedValueOnce({
      lastSyncedAt,
    } as never);
    (global.fetch as unknown as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
      new Response(JSON.stringify({ name: "users/1" }), {
        status: 200,
      }),
    );
  ...
  it("returns 422 when no Fitbit connection exists", async () => {
  ...
  it("classifies 403 upstream as credentials_rejected", async () => {
    vi.mocked(getValidToken).mockResolvedValueOnce({
      accessToken: "plain-access",
      connection: { id: "fc-1", fitbitUserId: "fu-1" },
    });
