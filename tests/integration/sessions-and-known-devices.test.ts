...
import { destroyOtherSessions, destroySessionById } from "@/lib/auth/session";
...
describe("destroySessionById", () => {
  it("revokes an owned session and refuses a cross-user id", async () => {
    ...
    // Cross-user attempt: owner tries to delete the intruder's session.
    expect(await destroySessionById(owner.id, otherSession.id)).toBe(false);
    expect(
      await prisma.session.findUnique({ where: { id: otherSession.id } }),
    ).not.toBeNull();

    // Owned delete succeeds.
    expect(await destroySessionById(owner.id, ownerSession.id)).toBe(true);
    expect(
